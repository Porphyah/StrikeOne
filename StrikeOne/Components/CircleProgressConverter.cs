...
namespace StrikeOne.Components
{
    ...
    public class CircleProgressValueConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            bool Clockwise = (Orientation) values[6] == Orientation.Horizontal;
            ...
            if (Value < Maximum)
            {
                ...
                Point Firstpoint = GetPointByAngel(CenterPoint, BigR, 0, Clockwise);
                Point Secondpoint = GetPointByAngel(CenterPoint, BigR, Angel, Clockwise);
                Point Thirdpoint = GetPointByAngel(CenterPoint, SmallR, 0, Clockwise);
                Point Fourthpoint = GetPointByAngel(CenterPoint, SmallR, Angel, Clockwise);
        ...
        private Point GetPointByAngel(Point CenterPoint, double r, double angel, bool Clockwise)
        {
            return new Point
            {
                X = Clockwise ? 
                    Math.Sin(angel * Math.PI / 180) * r + CenterPoint.X :
                    -Math.Sin(angel * Math.PI / 180) * r + CenterPoint.X,
                Y = Clockwise ? 
                    CenterPoint.Y - Math.Cos(angel * Math.PI / 180) * r :
                    CenterPoint.Y + Math.Cos(angel * Math.PI / 180) * r
            };
        }
